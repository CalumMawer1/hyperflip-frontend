    ...
    <div className="w-full mt-4 card-glow">
      <div className="border-b border-primary-dark py-3 px-4 sm:px-6 bg-gradient-to-r from-primary-light to-transparent">
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center">
          ...
          {isConnected ? (
            playerRank !== undefined && playerRank !== null ? (
              <PlayerRank 
            ...
            <div className="text-xs text-gray-400">Connect wallet to see your rank</div>
          )}
        </div>
      </div>
        ...
        <table className="w-full border-collapse text-sm" style={{ minWidth: "680px" }}>
          ...
          <tbody>
            {players?.map((player, index) => {
              ...
              const playerRowRank = sortBy === 'net_gain' 
                ? player?.rank_by_net_gain 
                : player?.rank_by_total_bets;
                
              const absoluteRank = playerRowRank || ((currentPage - 1) * itemsPerPage + index + 1);
